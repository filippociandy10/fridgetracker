...
export default function FridgeItemList() {
  ...
  const filteredItems = items.filter(item => {
    ...
    // Category filter
    if (filter.category && item.categoryId !== parseInt(filter.category)) {
      return false;
    }
    
    // Status filter
    if (filter.status) {
      if (filter.status === 'expired' && item.statusText !== 'Expired') {
        return false;
      }
      if (filter.status === 'expiring' && item.statusText !== 'Expiring Soon') {
        return false;
      }
      if (filter.status === 'good' && item.statusText !== 'Good') {
        return false;
      }
    }
                ...
                {filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link href={`/fridge-items/${item.id}`} className="text-blue-600 hover:text-blue-900 mr-4">
                        View
                      </Link>
                      <Link href={`/fridge-items/edit/${item.id}`} className="text-yellow-600 hover:text-yellow-900">
                        Edit
                      </Link>
                    </td>
                  </tr>
